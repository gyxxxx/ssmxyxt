			...
			<td>
				<table width="100%" border="0" align="center" cellpadding="4"
					...
					<tr align="left" bgcolor="#F2FDFF">
						<td height="30" colspan="17" background="images/table_header.gif">会员注册列表</td>
					</tr>
				</table>
			</td>
		</tr>
			...
			<td bgcolor="#FFFFFF">
								...
								<tr align="left">
									<td colspan="17">
										<form action="yonghuzhuceList.do" name="myform" method="post">
											查询 用户名：<input name="yonghuming" type="text" id="yonghuming"
												class="form-control2" /> 姓名：<input name="xingming"
												type="text" id="xingming" class="form-control2" /> 性别：<select
												name='xingbie' id='xingbie' class="form-control2"><option
													value="">所有</option>
												<option value="男">男</option>
												<option value="女">女</option></select> 出生年月：<input
												name="chushengnianyue1" type="text" id="chushengnianyue1"
												value='' onClick="WdatePicker({'dateFmt':'yyyy-MM-dd'})"
												class="form-control2" />-<input name="chushengnianyue2"
												type="text" id="chushengnianyue2" value=''
												onClick="WdatePicker({'dateFmt':'yyyy-MM-dd'})"
												class="form-control2" /> QQ：<input name="QQ" type="text"
												id="QQ" class="form-control2" /> 手机：<input name="shouji"
												type="text" id="shouji" class="form-control2" /> 身份证：<input
												name="shenfenzheng" type="text" id="shenfenzheng"
												class="form-control2" /> <input type="submit" value="查询"
						...
						<td style="padding-left: 0px; padding-right: 0px;"><table
								...
								<tr>
									...
									<td width="150px" height="30" align="center" bgcolor="#ebf0f7">
										添加时间</td>
									<td width="200" height="30" align="center" bgcolor="#ebf0f7">
										操作</td>
								</tr>
								<%
					int i=0;
				...
				%>
									<tr align="center" bgcolor="#FFFFFF">
										...
										<td width='90' align='center'><a href='${u.touxiang}'
											target='_blank'><img src='${u.touxiang}' width=88
												height=99 border=0 /></a></td>
										<td>${u.dizhi}</td>

										<td align='center'><input class="btn btn-info btn-small"
											type="button" value="${u.issh}"
											onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=yonghuzhuce'" /></td>
										<td align="center">${u.addtime }</td>
										<td width="200" align="center"><a
											class="btn btn-info btn-small"
											href="doUpdateYonghuzhuce.do?id=${u.id }">编辑</a> <a
											class="btn btn-info btn-small"
											href="deleteYonghuzhuce.do?id=${u.id }"
											onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a>
											<a class="btn btn-info btn-small"
											href="yonghuzhuceDetail.do?id=${u.id}">详细</a></td>
									</tr>
								</c:forEach>
							</table></td>
					</tr>
	...
	<p align="center">
		<c:if test="${sessionScope.p==1 }">
			<c:if test="${page.page>1}">
				...
				<a href="yonghuzhuceList.do?page=${page.page-1 }"> 上一页</a>
			</c:if>
			<c:if test="${page.page<page.totalPage}">
				<a href="yonghuzhuceList.do?page=${page.page+1 }">下一页</a>
				<a href="yonghuzhuceList.do?page=${page.totalPage }">末页</a>
			</c:if>
		</c:if>
	</p>
