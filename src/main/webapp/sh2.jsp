...
<body>
	<form action="sh2.jsp?ac=1" name="form1" method="post">
		<table width="32%" height="193" border="1" align="center"
			cellpadding="0" cellspacing="0" bordercolor="9DC9FF"
			...
			<tr>
				...
				</select> <input name="id" type="hidden" id="id"
					value="<%=request.getParameter("id")%>"><input
					name="tablename" type="hidden" id="tablename"
					value="<%=request.getParameter("tablename")%>"></td>
			</tr>
			<tr>
				<td>审核回复：</td>
				<td height="80"><textarea name="shhf" cols="50" rows="5"
						id="shhf"><%=connDbBean.readzd(request.getParameter("tablename"),"shhf","id",request.getParameter("id"))%></textarea></td>
			</tr>
	...
	document.form1.issh.value="<%=connDbBean.readzd(request.getParameter("tablename"),"issh","id",request.getParameter("id"))%>";
			...
			<tr>
				<td colspan="2" align="center"><input name="Submit"
					type="submit" value="确定" /> <input name="Submit2" type="reset"
					value="重置" /></td>
			</tr>
		</table>
	</form>
